...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tabsPanels = document.querySelector('.tabs-panels');
  ...
  // Đo chiều cao thật của một panel (kể cả khi đang bị hidden) để gán cho container,
  // nhờ đó .tabs-panels có thể transition chiều cao mượt mà thay vì snap đột ngột
  // khi nội dung 2 khu vực (HCM / Hà Nội) dài ngắn khác nhau.
  function measurePanelHeight(panel) {
    ...
    if (wasHidden) {
      // Tạm hiện panel ngoài luồng để đo, không ảnh hưởng layout đang thấy
      panel.hidden = false;
  ...
  function setTabsPanelsHeight(panel) {
    if (!tabsPanels || !panel) return;
    tabsPanels.style.height = measurePanelHeight(panel) + 'px';
  }
  ...
  function switchTab(btn) {
    ...
    // Khoá chiều cao hiện tại trước (tránh nhảy giật khi bắt đầu transition),
    // rồi animate mượt sang chiều cao thật của panel sắp hiện ra.
    if (tabsPanels && currentPanel) {
      tabsPanels.style.height = currentPanel.scrollHeight + 'px';
      void tabsPanels.offsetHeight; // force reflow
    }
    if (nextPanel) {
      setTabsPanelsHeight(nextPanel);
    }
  ...
  if (activeTab) {
    ...
    window.addEventListener('resize', () => {
      ...
      // Nội dung có thể xuống dòng khác khi đổi kích thước màn hình (vd. mobile),
      // nên cập nhật lại chiều cao container theo panel đang hiển thị (không animate).
      const activePanel = document.querySelector('.tab-panel.is-active');
      if (tabsPanels && activePanel) {
        const prevTransition = tabsPanels.style.transition;
        tabsPanels.style.transition = 'none';
        tabsPanels.style.height = activePanel.scrollHeight + 'px';
        void tabsPanels.offsetHeight;
        tabsPanels.style.transition = prevTransition;
      }
    });
  }

  // Gán chiều cao ban đầu cho .tabs-panels theo panel đang active (tránh container
  // bị sai chiều cao / giật khi người dùng đổi tab lần đầu tiên).
  if (tabsPanels) {
    const initialActivePanel = document.querySelector('.tab-panel.is-active');
    if (initialActivePanel) {
      requestAnimationFrame(() => {
        tabsPanels.style.height = initialActivePanel.scrollHeight + 'px';
      });
    }
  }
  ...
  /* ---------- 9. Form liên hệ (gửi qua /api/contact — serverless function trên Vercel,
     có honeypot + kiểm tra thời gian điền + rate limit theo IP ở phía server;
     function này forward tiếp sang Formspree nếu hợp lệ) ---------- */
  const contactForm = document.getElementById('contactForm');
  const formStatus = document.getElementById('formStatus');
  const formRenderedAtInput = document.getElementById('formRenderedAt');

  // Ghi lại thời điểm form xuất hiện trên trang — server dùng để phát hiện
  // submit "quá nhanh" (đặc trưng của bot điền form tự động).
  if (formRenderedAtInput) formRenderedAtInput.value = String(Date.now());
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', (e) => {
      ...
      // --- Honeypot (client-side): nếu trường bẫy có giá trị, coi như bot ---
      // Âm thầm báo "đã gửi" để không cho bot biết bị phát hiện, nhưng không thực sự gửi đi.
      const honeypotValue = contactForm.elements.company ? contactForm.elements.company.value : '';
      if (honeypotValue) {
      ...
      // --- Kiểm tra thời gian (client-side): submit quá nhanh sau khi trang tải xong = nghi bot ---
      const MIN_SUBMIT_TIME_MS = 2000;
      const renderedAt = formRenderedAtInput ? Number(formRenderedAtInput.value) : 0;
      ...
      // Gửi thẳng tới Formspree (xem action của form) — Formspree hỗ trợ AJAX cross-origin
      // khi header Accept là application/json.
      fetch(contactForm.action, {
        ...
        .then(async (response) => {
          ...
          const errorMessage = Array.isArray(data.errors)
            ...
            : null;
          formStatus.textContent = errorMessage || 'Có lỗi xảy ra, vui lòng thử lại hoặc liên hệ qua email.';
        })
        .catch(() => {
  ...
  function typeIntoElement(el, html, speed = 30) {
    return new Promise((resolve) => {
      ...
      function step() {
        ...
        if (html[i] === '<') {
          // Nhảy qua nguyên một thẻ HTML (vd. <a href="#...">) thay vì gõ từng ký tự bên trong,
          // tránh hiện thẻ lỗi/dang dở giữa chừng.
  ...
  async function sendMessage() {
    ...
    // Get AI response (askAI returns HTML string)
    let botResponseHTML = 'Xin lỗi, có lỗi xảy ra.';
    try {
      if (askAI) {
        botResponseHTML = await askAI(messageText);
      } else {
        botResponseHTML = 'Chatbot đang được tải, vui lòng thử lại sau giây lát.';
      }
    } catch (err) {
      console.error(err);
      botResponseHTML = 'Đã có lỗi xảy ra khi xử lý tin nhắn.';
    }

    // Remove typing indicator
    typingDiv.remove();

    // Tạo khung tin nhắn bot rồi gõ chữ từ từ thay vì hiện hết ngay lập tức
    const botMsgDiv = document.createElement('div');
    botMsgDiv.className = 'chat-message bot';
    chatboxMessages.appendChild(botMsgDiv);
    await typeIntoElement(botMsgDiv, botResponseHTML);
    ...
    botMsgDiv.querySelectorAll('a[href^="#"]').forEach(link => {
        link.addEventListener('click', e => {
            ...
            // Nếu link trỏ tới nội dung nằm trong 1 tab cụ thể (vd. danh sách bệnh viện
            // TP.HCM / Hà Nội), chuyển sang đúng tab đó trước khi cuộn tới.
  ...
  function sendGreeting() {
    ...
    typeIntoElement(botDiv, greeting);
  }
