...
function buildNavButtons(sourceNodeIds, engine) {
  ...
  for (const id of sourceNodeIds) {
    ...
    const tabHint = node.category.startsWith('co-so-dieu-tri') ? guessHospitalTab(node.id) : '';
    buttons.push(
      '<a href="' + node.source + '" class="chat-nav-btn" data-node-id="' + escapeAttr(node.id) + '"' +
      (tabHint ? ' data-tab="' + tabHint + '"' : '') +
      (node.category === 'faq' || node.category.startsWith('faq:') ? ' data-faq-title="' + escapeAttr(node.title) + '"' : '') +
